...
**Как собрать** (нужна модель на GPU — делается прямо в ComfyUI, второй torch не
нужен): поставь ноду **XQuant imatrix: Capture** перед `KSampler` и **Save** после,
прогони 1–3 генерации на разных промптах → получишь `<model>.imatrix.npy`. Укажи
его в `XQuant.exe` (поле 🎯 imatrix) или `XQUANT_IMATRIX=<путь>`. Композится с
